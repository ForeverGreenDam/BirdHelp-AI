...
3. layout_type 有效性: layout_type 是否为有效值（cover, section, text_only, text_image, image_full, two_column, grid_cards, timeline, big_number, chart, table, quote, summary）？无效值阻塞
...
{{
  ...
  "checks": [
    ...
    {{"label": "layout_type有效性", "passed": true, "severity": "blocking", "detail": ""}},
...
class PptQAChain:
    ...
    async def _repair_slide(
        ...
        repair_prompt = (
            ...
            f"7. body/content 必须是字符串数组: [\"要点1\", \"要点2\", ...]，绝不能是单个字符串\n"
            f"8. layout_type 必须是以下之一: cover, section, text_only, text_image, "
            f"image_full, two_column, grid_cards, timeline, big_number, chart, table, quote, summary"
        )
        ...
        try:
            ...
            # 确保 body 是列表格式（LLM 可能输出单个字符串）
            body = repaired.get("body", repaired.get("content"))
            if isinstance(body, str):
                repaired["body"] = [body]
            elif isinstance(body, list):
                repaired["body"] = body
            # 校验并修复无效的 layout_type
            VALID_LAYOUTS = {
                "cover", "section", "text_only", "text_image",
                "image_full", "two_column", "grid_cards",
                "timeline", "big_number", "chart", "table", "quote", "summary",
            }
            if repaired.get("layout_type", "") not in VALID_LAYOUTS:
                logger.warning(
                    f"QA repair produced invalid layout_type "
                    f"'{repaired.get('layout_type')}', falling back to text_only"
                )
                repaired["layout_type"] = "text_only"
