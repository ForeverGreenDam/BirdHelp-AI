...
以上所有接口均需 RSA-SHA256 签名验证，与现有 `JAVA_CALLER.md` 一致。Python 侧已有完整的签名 HTTP 客户端实现（`client/http.py`）。
...
### Python 端（已完成 2026-05-23）
- [x] 在 `config.py` 新增 RabbitMQ 配置项 — 9 个字段（host/port/vhost/user/password/exchange/queue/prefetch）
- [x] 在 `.env` 新增 RabbitMQ 环境变量（见 9.3 节）
- [x] 实现 `aio-pika` 消费者启动逻辑 — `broker/consumer.py` `GenerationConsumer.start()`
- [x] 实现消息校验（version / docType / 必填字段）— `broker/consumer.py` `_parse_and_validate()` + `broker/schemas.py` `TaskMessage`
- [x] 实现消费流程（额度扣减 → 生成 → 上传 → 回调）— `_process_message()` 5 阶段流水线
- [x] 实现重试计数（x-retry-count header）— `_get_retry_count()` + `_re_enqueue()`，max 3 次
- [x] 实现进度推送 — `client/task.py` `progress()` + `broker/schemas.py` `TaskProgress`
- [x] 连接断开自动重连 — `aio_pika.connect_robust()` 内置自动恢复
- [x] 消费完成后 ACK，不可重试错误 NACK(requeue=false)，可重试错误 NACK(requeue=true)
- [x] 移除旧的 `/ai/ppt/generate`、`/ai/word/generate`、`/ai/pdf/generate` 同步接口 — `api/{ppt,word,pdf}.py` + `services/generation.py` 已删除
- [x] 调用新的 `/api/internal/task/callback` 替代旧的返回文件信息方式（含 RSA 签名，与现有 `client/http.py` 一致）— `client/task.py` + `broker/consumer.py` `_send_success_callback()` / `_send_failure_callback()`
- [x] 调用 `/api/internal/task/progress` 推送进度（RSA 签名，格式见 4.4 节）— `client/task.py` `progress()`
